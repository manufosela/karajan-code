...
## Fase 118: v4.26.0 — Un gobierno que se puede leer

El mismo día en que salió la release de la ventana única, el gobierno se miró a sí mismo. Las excepciones ganaron ámbito declarado — proyecto o máquina entera — con procedencia estampada desde el almacén físico del que se leyó cada registro, infalsificable editando texto; la fusión vive en el único punto de carga, de modo que los invariantes de composición del ADR 0006 evaluarán la unión completa gratis. Cada mensaje del Sentinel se firma ya como karajan y enlaza una página de documentación gate por gate, nacida de las palabras del propio usuario: «no entiendo los mensajes — un enlace ayudaría». Y el HU-Board absorbió dos órdenes directas del usuario la misma tarde en que las dio: nombres de proyecto únicos derivados del repo (un duplicado es un error alto, nunca un gemelo) y una navegación en dos niveles que deja de disfrazar las vistas de proyecto de botones globales. El cauce sancionado para versionar lo que `kj harden` regenera — lo único que la clase de seguridad se niega, con razón, a dejar mergear incluso a su autor — queda propuesto como ADR 0009, para que decida el usuario.

## Fase 119: v4.27.0 — El cauce humano, a fuego real

El usuario convirtió la última pregunta abierta del gobierno en una orden: probar que la IA no puede versionar los ficheros del propio supervisor — ni siquiera cuando se le ordena. El ADR 0009 (opción A) responde con un cauce humano por construcción: `kj harden --commit` sella la procedencia en el acta encadenada y commitea quirúrgicamente, y los gates solo alzan diffs de supervisor contra procedencia sellada más el render canónico que el propio kj recompute. Las cuatro capas de «humano» se forjaron adversarialmente esa misma tarde: la denegación en sesión se autoprobó bloqueando a la sesión de su autor; el chequeo de terminal; la ascendencia de procesos que un pty falso no puede quitarse de encima; y — después de que un doble fork huérfano atravesara de verdad las capas en el sandbox — el nonce aleatorio tecleado en la tty real de control. Cinco clases de ataque, cero commits, y el residual documentado en vez de escondido: la garantía no es metafísica — es que no existe camino sin engaño premeditado multicapa, que cada intento deja rastro, y que ninguna falsificación puede hacer al guardia más débil que lo que kj genera.

## Fase 120: v4.28.0 — La llave la tiene el móvil

El sello del supervisor gana la capa que el usuario pidió por su nombre: una firma desde un dispositivo que el agente jamás tocará. Una página firmante genera un par Ed25519 en el móvil (privada no-extraíble), la pública se enrola en kj, y cada sello es un desafío QR que el móvil firma tras mostrar exactamente qué firma — verificado contra la clave enrolada, obligatorio una vez enrolado, probado de punta a punta contra producción. Esa misma noche, la atribución a IA pasó a ser un bloqueo determinista en toda superficie (contenido del commit, cuerpo/título de PR y todo `gh` que publica) tras un default del harness que coló pies por delante de un guard que solo miraba commits; y aterrizaron dos guards de sesión nacidos del campo — un deny en seco de `git worktree remove` sobre el árbol principal, y el enrolamiento del móvil solo humano. El único residual se nombra, no se esconde: la firma se verifica en local, y la verificación server-side (en CI) que la haría independiente de un kj sin parchear queda planificada.

## Fase 121: v4.29.0 — Sonar no se esquiva

El análisis estático deja de ser disciplina y pasa a ser un gate de git. El detonante fue real: una card llegó a `main` en cuatro PRs con `sonarqube.enabled: false` en su configuración y nada la paró. Ahora el resultado de Sonar viaja dentro del registro del veredicto, atado al hash exacto del diff como el propio veredicto, y el pre-commit lo exige; la cobertura se prueba contra el índice de ficheros del propio scanner en vez de inferirse de la configuración, así que una fuente staged que el análisis no vio bloquea. Sonar deshabilitado, caído o parcial rechaza cualquier diff con código (solo docs pasa), ninguna variable de entorno lo levanta, y la única excepción es un `kj policy grant` humano y con caducidad. Los monorepos escanean cada fichero staged desde su `sonar-project.properties` más cercano con la clave de ese proyecto; `kj doctor` marca `enabled: false` como defecto de configuración; el informe del método clasifica los veredictos recientes en probados, solo-docs, concedidos o sin probar. Junto al gate, tres arreglos del MCP reportados desde el campo la misma semana: las herramientas directas aceptan `taskFile`, `kjHome` llega al bootstrap gate mediante un home por contexto async en `karajan-core` 1.5.0 (sin mutar el entorno del proceso, así que las llamadas solapadas nunca se contaminan), y `kj_rag_query` acota la búsqueda al proyecto que llama.

## Fase 122: v4.30.0 — El RAG como gate

Los bugs del 16 de septiembre tenían una misma forma: un concepto que vive en dos sitios y un cambio que solo llega a uno, porque la sesión supuso en vez de preguntar al código. El ADR 0010 convirtió el RAG de costumbre en gate. El Sentinel lleva un registro por sesión de cada consulta al RAG y de las fuentes que devolvió, tanto por CLI como por MCP, y rechaza editar un fichero fuente sobre el que la sesión nunca preguntó, él mismo o un hermano de su directorio, con una sola consulta bastando para un fichero nuevo; el escape es una excepción sellada, una vez por sesión, que la review nunca acepta. La review staged rechaza código que el registro no cubre antes de gastar un token del revisor, le entrega los gemelos que el RAG devolvió y el diff no toca, y guarda un bloque `rag` en el veredicto atado al hash del diff; el pre-commit lo exige, tratando su modo como afirmación y no como autorización, cerrándose ante un harness sin verificar y sellando un aviso para los veredictos del pipeline, que aún no llevan registro. El informe del método cuenta la prueba RAG de los veredictos recientes y se pone en rojo con código sin probar, y `kj check` falla con fuentes que el índice no ve, porque un gate sobre el RAG no vale nada sobre un índice que no cubre el código. Construir el gate destapó que la aprobación de Solomon escribía un veredicto sin los bloques de Sonar y del RAG, así que todo diff arbitrado se rechazaba al commitear desde 4.29.0. Alrededor, las superficies que el campo seguía rompiendo recibieron su propia prueba: el gate del tarball ejercita ahora el servidor MCP por stdio con `kjHome` y `taskFile`, un test de paridad ata cada parámetro del schema MCP a un handler que lo lee, y `kj audit` informa de claves resueltas con reglas distintas en distintos módulos, justo el patrón de las migraciones a medias.

## Fase 123: v4.31.0 — El arranque en frío deja de exigir forzar

Todos los gates se habían construido desde dentro de un repositorio maduro, y eso se notó en cuanto alguien empezó desde cero. Un aviso de campo sobre un proyecto nuevo abrió el mapa del camino entero desde una carpeta vacía, y no había un muro, había nueve. El peor era mudo: sin ningún commit, el Stop gate le pedía a git un diff contra un HEAD que no existía, fallaba en prudente y exigía un test para un código que no lo tenía, turno tras turno, sin que ningún escape llegara hasta él. Al lado, el commit que instala el contrato lo rechazaba el guard de rama, porque una rama sin commits sigue leyéndose como `main`; `kj init` en un directorio sin repositorio se saltaba el harness entero y aun así anunciaba éxito; un harden headless dejaba el clon sin identidad y el Sentinel denegaba después todo `git` y todo `gh` sin que nada lo conectara con aquello; el gate de cards mandaba al agente a crear una card cuyo identificador generado rechazaba ese mismo gate; rag-first denegaba ediciones que nadie podía satisfacer mientras el índice estuviera vacío; y `kj go`, la puerta de quien no tiene oficio, no podía arrancar en una carpeta vacía. La respuesta no fue relajar el método, sino nombrar la fase: el arranque se define por un hecho que nadie puede falsear, el repositorio no tiene ningún commit, y termina en el primero. En sentido contrario, la misma tanda cerró un agujero que apuntaba a lo mismo que la épica del cumplimiento: un harness ausente abría el gate del RAG, lo que convertía no hardenar en la forma cómoda de esquivarlo, y ahora bloquea con las dos salidas honestas, instalarlo o pedir un permiso humano.

## Fase 124: v4.32.0 — Se empieza sin forzar nada

La fase 123 dibujó los nueve muros del arranque en frío; esta los derriba. `kj bootstrap` impone el orden que nadie debería tener que recordar, y hace él el commit del contrato: el commit que `project-new.md` llevaba pidiéndole al USUARIO era justo el que sus gates recién instalados rechazaban, así que la persona acababa eligiendo entre forzar un gate y rendirse. El detector cierra la otra mitad: por primera vez un check mira el repositorio en sí, y no las herramientas y los puertos, y cada síntoma que nombra viene con el mando literal que lo repara, porque un diagnóstico sobre el que nadie puede actuar es una queja. Después, leer el artículo de Anthropic sobre harnesses para agentes de larga duración destapó dos huecos que la propia minuciosidad del método tapaba. Karajan verificaba el método a conciencia y nunca verificaba que la aplicación arranca, así que una sesión podía pasarse horas construyendo sobre un árbol ya roto; ahora el proyecto lo declara en una línea, antes de tocar nada, y un fallo anterior al trabajo se dice en vez de achacárselo al cambio siguiente. Y una card cuyo resultado VE una persona se cerraba con la suite verde, que es la misma falsa confianza que el artículo observó en campo: el recorrido viaja ahora dentro del veredicto, atado al diff como las pruebas de Sonar y del RAG. Las dos llegan avisando y sin bloquear, a propósito, porque el navegador no puede ver un modal nativo y un gate que salta a menudo enseña a saltárselo, que es la lección que todas las demás reglas de aquí aprendieron a golpes.

## Fase 125: v4.33.0 — Tu panel manda

Elegir quién escribe y quién revisa se había quedado en una preferencia que el código solo respetaba dentro de `kj run`. Con el agente anfitrión orquestando, que es como funciona la v4, nada llevaba esa elección a la sesión: el anfitrión escribía el código y el usuario se enteraba por casualidad. Así que el panel viaja ahora con el playbook que todo anfitrión lee al empezar, anunciando y sin bloquear, y `kj code` deja de entregarle al coder elegido una frase pelada: le lleva la card, sus criterios como contrato, la frontera del proyecto y lo que el índice responde sobre la tarea, porque un coder lanzado como subproceso no puede consultar el RAG por su cuenta y lo que hacía era suponer. Debajo, un hallazgo que merece quedar escrito: el esquema de una cadena declarada de modelos llevaba en la config desde febrero, recursivo y con su modelo, y el envoltorio de recuperación siempre supo recorrerlo. Nadie construyó jamás la pieza intermedia, así que toda cadena que alguien declarase era config muerta, mientras cinco agentes guardaban cada uno un reintento privado que resolvía un modelo retirado en silencio y POR DELANTE de lo que el usuario hubiera declarado. Ahora los agentes devuelven el fallo, decide la cadena, el default del proveedor es su cola y no un primer paso escondido, y una cadena agotada cuenta qué se probó y en qué orden. kj además recuerda el cadáver para que la siguiente ejecución no vuelva a pagarlo, sin tocar el pin, porque el pin es del usuario. Y tres informes de campo de un contribuidor cerraron la mitad que desde dentro del proyecto no se veía: lo que kj escribe en un proyecto era JavaScript en cualquier lenguaje, commitlint imponía un runtime de Node a stacks que habían elegido no tenerlo, y el commit que instala el harness lo rechazaba el guard que el propio harness impone.

## Fase 126: v4.34.0 — Un gate que bloquea su propio arreglo

Un día corriendo el método contra su propio autor destapó nueve gates que bloqueaban justo la acción que los habría puesto en verde. El release check exigía la landing desplegada con la versión nueva y el guard trataba ese despliegue como una publicación, así que llegar a él pasaba por `KJ_ALLOW_RELEASE=1`, que no levanta ese item: apaga el gate entero, incluida la protección sobre el publish de treinta segundos después. board-sync se negaba a dejar pasar la segunda mitad de una card después de haber aceptado que las cards se parten, porque registraba un pendiente por PR mergeada y una card no es una PR. El supervisor leía su propio desarrollo como manipulación, ya que la detección comparaba los guardias instalados contra las plantillas que se estaban editando, y el comando con el que se desbloquea eso, `kj harden`, reescribía los guardias y destruía el sello que habría hecho innecesario volver a usarlo. Y un `*/` suelto en un fichero fuente se llevó por delante al evaluador de policy, con lo que el guard denegó Edit, Write y Bash a la vez, que es exactamente el juego de herramientas con el que se arregla el fichero, mientras señalaba un comando que tampoco arrancaba.

La forma del arreglo es la misma en todos, y merece nombrarse porque no es "ablandar el gate". Un gate tiene que distinguir un comando que CONSUME su estado en rojo de uno que lo REPARA, y esa distinción tiene que apoyarse en un hecho comprobable en lugar de en la palabra del agente: un item que declara `remedied_by`, otra PR abierta de la misma card, un hash que un humano selló, el fichero que nombra un diagnóstico. Donde faltaba el hecho, el gate sigue bloqueando, porque inventarse el permiso es peor que un bloqueo. Cada levantamiento se dice además en voz alta, porque una exención que nadie ve no se distingue de un gate que dejó de mirar, y el Stop gate queda intacto en todo esto: el turno sigue sin poder terminar con trabajo sin registrar. Por debajo, el mismo día cerró una cuarta implementación del presupuesto de LOC, que imprimía 174 y 202 para el mismo diff en dos líneas seguidas, y una regresión que borraba seis líneas de reglas del `CLAUDE.md` de cualquier proyecto TypeScript mientras informaba "updated".
