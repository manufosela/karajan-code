...
## Phase 113: v4.16.0–v4.21.0 — One roof, one kernel, and gates that outlive memory

Six minors that turned governance from features into structure. The family moved into one monorepo with full histories and lane isolation enforced before the damage (v4.17.0). Governance became a package — `@karajan-family/governance`, policy/decision/exception as first-class objects on a hash-chained, git-anchored log — with kj as its first consumer, not its owner (v4.18.0), and exceptions that EXPIRE on their own: the burden of proof inverted (v4.18.0). Green stopped being proof: the mutation pre-gate ships survivors to the reviewer or blocks before spending a token (v4.19.0). Infra became source: terraform/k8s/shell/Docker edits count for card-first and the gates, with real validation suites and checkov in the security audit (v4.20.0). And two rules left the agent's memory for good: identity lock (every clone declares who works it; the Sentinel refuses another account BEFORE the command runs) and the board-sync gate (a merged card must move in the tracker before anything advances) — both born from real incidents (v4.21.0). The first dual-publish under `@karajan-family/code` shipped in v4.19.0. Details per release live in the [CHANGELOG](https://github.com/manufosela/karajan-code/blob/main/CHANGELOG.md).

## Phase 114: v4.22.0 — Nothing without evidence

Two fronts, one idea: claims need sources. The **Karajan Console** joined the family (`@karajan-family/console`) as its most privileged piece — access, health, operations and credentials of a deployed instance, every action sealed on the same governance kernel, identity verified server-side behind Google Sign-In or a fully-terraformed Identity-Aware Proxy, and the second layer never trusting the first. Its shape was carved by a real instance's field reports within days of each publish. And **claims with evidence**: the session transcript is already the register of sources, so `kj claims check` deterministically crosses every hard datum the AI states (counts, versions, paths, ids, SHAs) against the outputs of that same turn — a datum *denied by its own source* is a proven hallucination and blocks the turn. The same cycle hardened the method against its own author: the review-verdict parser was fixed at the path it actually walks after a green test on the wrong function let the bug survive, and the repo's own workflows caught up with the SHA-pinning its generator already practiced.

## Phase 115: v4.23.0 — Trust expires

Karajan learns to govern the project's STATE, not just its acts. The **Steward** ships whole: invariants with freshness answer one of FOUR verdicts — ok, broken, unknown (evidence expired → refresh) and not-observable (nowhere to look → instrument) — calibrated with a real project's measured decay (79 days without a security audit; 21 days of red E2E hiding a 17-day production bug behind "how long has main been red" having no possible answer). `kj steward sweep` versions the verdict in the repo, seals it in the decision chain, and cards each break as PROPOSED work that nothing executes unreviewed; consequence is proportionate by design — inform always, impossible to miss, block almost never. Phantom coverage gets its two detectors (unit call-graph; E2E literal crossing), the credibility epic removes every measured false positive from the gates (member reachability with an honest perimeter, derivative-first dead-code reports, delete-only diffs owing no test, sonar vetoing only touched lines, budgets ignoring generated weight, privacy scan with context), and claims-with-evidence reaches written artifacts: a PR body with a datum denied by its own source is refused before the PR exists. An external contributor made the HU Board token comparison constant-time — the first security PR from outside the family.

## Phase 116: v4.24.0 — Muggles welcome
...
## Phase 119: v4.27.0 — The human channel, under fire

The user turned the last open question of the government into an order: prove that the AI cannot version the supervisor's own files — not even when ordered to. ADR 0009 (option A) answers with a channel that is human by construction: `kj harden --commit` seals provenance into the chained log and commits surgically, and the gates lift supervisor diffs only against sealed provenance plus the canonical render kj itself recomputes. The four layers of "human" were forged adversarially the same evening: the in-session denial self-proved by blocking its author's session; the tty check; process ancestry that a faked pty cannot shake off; and — after an orphaned double-fork genuinely broke through in the sandbox — a random nonce typed back on the real controlling tty. Five attack classes, zero commits, and the residual documented instead of hidden: the guarantee is not metaphysics, it is that no path exists without premeditated multi-layer deception, every attempt leaves a trail, and no forgery can ever make the guard weaker than what kj generates.

## Phase 120: v4.28.0 — The phone holds the key

The supervisor seal gets the layer the user asked for by name: a signature from a device the agent will never touch. A static signer page mints an Ed25519 keypair on the phone (private half non-extractable), the public half is enrolled in kj, and every seal is a QR challenge the phone signs after showing exactly what it is signing — verified against the enrolled key, mandatory once enrolled, proven end-to-end against production. The same night, AI attribution became a deterministic block on every surface (commit content, PR body/title, and every publishing `gh` command) after a harness default slipped footers past a commit-only guard; and two field-born session guards landed — an outright deny of `git worktree remove` on the main tree, and human-only phone enrollment. The one residual is named, not hidden: the signature is verified locally, and the server-side (CI) verification that would make it independent of an unpatched kj is scoped as a follow-up.

## Phase 121: v4.29.0 — No way around Sonar

Static analysis stops being discipline and becomes a git gate. The trigger was real: a card reached `main` in four PRs with `sonarqube.enabled: false` in its config and nothing stopped it. Now the Sonar result travels inside the verdict record, bound to the exact diff hash like the verdict itself, and the pre-commit demands it; coverage is proved against the scanner's own file index rather than inferred from config, so a staged source the scan did not see blocks. Sonar disabled, down or partial refuses any diff with code (docs-only passes), no environment variable lifts it, and the only exception is a human, expiring `kj policy grant`. Monorepos scan each staged file from its nearest `sonar-project.properties` with that project's key; `kj doctor` flags `enabled: false` as a configuration defect; the method report classifies recent verdicts as proved, docs-only, granted or unproved. Alongside the gate, three MCP fixes reported from the field the same week: the direct tools honour `taskFile`, `kjHome` reaches the bootstrap gate through an async-context home in `karajan-core` 1.5.0 (no process env mutation, so overlapping calls never leak into each other), and `kj_rag_query` scopes its search to the caller's project.

## Phase 122: v4.30.0 — The RAG as a gate

The bugs of 16 September had one shape: a concept living in two places and a change reaching only one, because the session guessed instead of asking the codebase. ADR 0010 turned the RAG from a habit into a gate. The Sentinel keeps a per-session ledger of every RAG query and the sources it returned, CLI and MCP alike, and refuses an edit of a source file the session never asked about, itself or a sibling of its directory, with a new file needing one consultation; the escape is a sealed, once-per-session exception the review never honours. The staged review refuses code the ledger does not cover before any reviewer token, hands the reviewer the twins the RAG returned that the diff does not touch, and stores a `rag` block in the verdict bound to the diff hash; the pre-commit demands it, treating its mode as a claim rather than an authorization, failing closed on an unverified harness and sealing a warn for pipeline verdicts that carry no ledger yet. The method report counts the rag proof of recent verdicts and goes red on unproved code, and `kj check` fails on sources the index cannot see, because a gate on the RAG is worthless over an index that does not cover the code. Building the gate exposed that Solomon's approval wrote a verdict without the Sonar and RAG blocks, so every arbitrated diff had been refused at commit since 4.29.0. Around it, the surfaces the field kept breaking got their own proof: the tarball gate now exercises the MCP server over stdio with `kjHome` and `taskFile`, a parity test binds every MCP schema parameter to a handler that reads it, and `kj audit` reports keys resolved with divergent rules across modules, the very pattern of the half-way migrations.

## Phase 123: v4.31.0 — The cold start stops asking you to force it

Every gate had been built from inside a mature repository, and it showed the moment someone started from nothing. A field report on a brand new project opened the map of the whole path from an empty directory, and there were nine walls, not one. The worst was silent: with no commit yet, the Stop gate asked git for a diff against a HEAD that did not exist, failed conservative, and demanded a test for code that had none, every turn, with no escape reaching it. Next to it, the commit that installs the contract was rejected by the branch guard, because a branch with no commits still reads as `main`; `kj init` in a directory without a repository skipped the entire harness and still announced success; a headless harden left the clone with no identity and the Sentinel then denied every `git` and `gh` call with nothing tying it back; the card gate sent the agent to create a card whose generated id the same gate rejected; rag-first denied edits nobody could satisfy while the index was empty; and `kj go`, the door for someone with no craft, could not start in an empty folder at all. The answer was not to relax the method but to name the phase: the bootstrap is defined by a fact nobody can fake, the repository has no commit, and it ends at the first one. In the other direction the same pass closed a hole that pointed the same way as the enforcement epic: an absent harness used to open the RAG gate, which made not hardening the comfortable way around it, and now it blocks with the two honest ways out, install it or ask a human for a grant.

## Phase 124: v4.32.0 — You start without forcing anything

Phase 123 mapped the nine walls of the cold start; this one tears them down. `kj bootstrap` imposes the order nobody should have to remember, and makes the contract commit itself: the commit that `project-new.md` had been asking the USER for was the very one their freshly installed gates rejected, so the person was left arbitrating between forcing a gate and giving up. The detector closes the other half: for the first time a check looks at the repository itself rather than at tools and ports, and every symptom it names comes with the literal command that repairs it, because a diagnosis nobody can act on is a complaint. Then a reading of Anthropic's write-up on harnesses for long-running agents showed two holes that the method's own thoroughness had hidden. Karajan verified the method exhaustively and never verified that the application runs, so a session could spend hours building on a tree that was already broken; now the project declares that in one line, before anything is touched, and a failure that predates the work is said instead of being blamed on the next change. And a card whose result a person SEES was being closed on a green suite, which is the same false confidence the article observed in the field: the walkthrough now travels inside the verdict, bound to the diff like the Sonar and RAG proofs. Both arrive warning rather than blocking, on purpose, because the browser cannot see a native modal and a gate that fires often teaches people to skip gates — the lesson every other rule here learned the hard way.

## Phase 125: v4.33.0 — Your panel rules

Choosing who writes and who reviews had become a preference the code only honoured inside `kj run`. With the host agent orchestrating, which is how v4 works, nothing carried that choice into the session: the host wrote the code and the user found out by accident. So the panel now travels with the playbook every host reads at session start, announcing and never blocking, and `kj code` stops handing the chosen coder a bare sentence: it carries the card, its criteria as the contract, the project boundary and what the index answers about the task, because a coder launched as a subprocess cannot query the RAG itself and was guessing the codebase instead. Underneath, a finding worth recording: the schema for a declared chain of models had been in the config since February, recursive and carrying a model, and the recovery wrapper always knew how to walk it. Nobody ever built the piece in between, so every chain anyone declared was dead config, while five agents each kept a private retry that resolved a retired model silently and BEFORE anything the user had declared. The agents now return the failure, the chain decides, the provider default is its tail rather than a hidden first step, and an exhausted chain reports what was tried and in what order. kj also remembers the corpse so the next run does not pay for it again, without editing the pin, because the pin is the user's. And three field reports from a contributor closed the half nobody inside the project could see: what kj writes into a project was JavaScript in every language, commitlint imposed a Node runtime on stacks that had chosen not to have one, and the commit that installs the harness was rejected by the guard the harness itself enforces.

## Phase 126: v4.34.0 — A gate that blocks its own repair

A day of running the method against its own author turned up nine gates that blocked the one action that would have turned them green. The release check demanded a landing deployed with the new version and the guard treated that deploy as a publication, so reaching it meant `KJ_ALLOW_RELEASE=1`, which does not lift that item, it switches off the whole gate including the protection over the publish thirty seconds later. board-sync refused the second half of a card after accepting that cards get split, because it recorded one pending move per merged PR and a card is not a PR. The supervisor read its own development as tampering, since detection compared the installed guards against the templates being edited, and the command used to unblock that, `kj harden`, rewrote the guards and destroyed the seal that would have made unblocking unnecessary. And a stray `*/` in a source file took the policy evaluator down, so the guard denied Edit, Write and Bash at once, which is exactly the set of tools needed to repair the file, while pointing at a command that could not start either.

The shape of the fix is the same in all of them, and it is worth naming because it is not "make the gate softer". A gate has to distinguish a command that CONSUMES its red state from a command that REPAIRS it, and that distinction has to rest on a verifiable fact rather than on the agent's word: an item declaring `remedied_by`, another open pull request for the same card, a hash a human sealed, the file a diagnostic names. Where a fact was missing, the gate keeps blocking, because inventing permission is worse than a deadlock. Every lift is printed too, since an exemption nobody sees is indistinguishable from a gate that stopped looking, and the Stop gate is untouched throughout: the turn still cannot end with work nobody recorded. Underneath, the same day closed a fourth implementation of the LOC budget, which was printing 174 and 202 for one diff two lines apart, and a regression that erased six lines of rules from every TypeScript project's `CLAUDE.md` while reporting "updated".
