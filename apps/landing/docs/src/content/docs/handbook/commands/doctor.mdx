...
A second mode, `--project`, runs **only project-aware checks**: it inspects the current repo for stack signals, verifies the external tools each signal needs are present, checks write permissions, `.env` consistency, that the `gh` remote is reachable, the state of the repository itself (the order it was set up in, whether the contract travels to a clone, whether this clone declares an identity), and the **pinned GitHub Actions**.

That last one is the maintenance that pinning moved onto kj. The workflows `kj harden` writes pin their actions to a commit SHA, because a moving tag lets whoever controls an action run code with your repo's secrets. The trade is that nobody delivers the upstream patch either, so `kj doctor` asks GitHub what each tag points at today and warns with both SHAs and the literal line that updates the pin. It asks through `gh` when you have it (your own credentials, 5000 calls an hour) and falls back to an unauthenticated call, which GitHub caps at 60 an hour per IP. When it cannot reach the API it **says so** rather than reporting green: an unchecked pin is not a current pin. This check lives here and deliberately **not** in `kj check`, because a gate that needs the network is a gate that fails on a plane.
