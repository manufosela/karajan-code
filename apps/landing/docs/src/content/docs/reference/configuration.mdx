...
Any role accepts a `fallback`, and a fallback accepts its own, so the chain is as long as you need it (five links, then kj warns and ignores the rest):
...
roles:
  reviewer:
    ...
    model: gpt-5.6-terra
    fallback:
      provider: codex          # same provider, another model
      model: gpt-5.6
      max_wait_hours: 12
      fallback:
        provider: claude       # or another provider entirely
        model: null            # null = that provider's default
```
...
:::note
`coder_options.fallback_coder` and `reviewer_options.fallback_reviewer` are the older, single-step, provider-only form. They still work. `roles.<role>.fallback` is the one that covers every role and can change the model.
:::
...
What `kj release check` verifies on top of the generic checks (manifest against CHANGELOG against tags, a privacy scan of the publishable files, the policy in force). Every item is the project's own.
...
release_check:
  items:
    ...
    - name: the footer carries the version
      file_contains:
