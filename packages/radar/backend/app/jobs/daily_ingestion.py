...
async def run_pipeline(shutdown_event: asyncio.Event | None = None) -> None:
    """Run ingestion of all sources then the signal pipeline; raises on failure."""
    # Shared by the daily job and the run-now endpoint. It does not configure
    # logging, handle signals, or dispose the engine: the caller owns that
    # lifecycle (the API must never dispose its shared engine).
...
async def main() -> int:
    """Entry point for the daily ingestion job. Returns 0 on success, 1 on failure."""
    configure_logging()
    ...
    try:
        await run_pipeline(shutdown_event)
        return 0
    except Exception:
