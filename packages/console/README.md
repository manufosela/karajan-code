...
Audit sink for Cloud Run / Functions (ephemeral filesystem): `"audit": { "sink": "gcs-jsonl", "bucket": "<bucket>" }` — one immutable object per entry, chain rebuilt from the bucket at start, a refused upload is a 502 and never a sealed entry; entries are sealed one at a time per process, so concurrent requests never chain on the same predecessor — and `GET /api/audit` waits for that sealing queue to drain before answering, so reading right after a burst includes every sealed entry (a failed upload never hangs the query, and its entry is never counted as sealed). Run ONE console instance per bucket (`max_instances=1` on Cloud Run): the chain lives in the process and two instances would race each other. `file` and `memory` are for a VM or tests.
