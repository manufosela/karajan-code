...
`kj init` also starts the SonarQube container when Docker is available. Pass `--no-sonar` to skip it (CI, scripts, another agent bootstrapping the project) and run `kj sonar start` when you need it.
