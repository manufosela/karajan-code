...
- Configuracion via `kj.config.yml` o flags del CLI
...
Regla de diff para el reviewer (importante en equipos):
- Baseline por defecto: `merge-base(HEAD, origin/<base-branch>)` (default `main`).
- En `kj run`, se crea un checkpoint inicial de sesion (`session_start_sha`) y el diff del reviewer se calcula desde ese punto para evitar mezclar cambios externos.
- Si durante la sesion cambia la rama base remota, `kj` avisa y permite: continuar con baseline congelado o refrescar baseline.
- Override manual: `--base-ref <sha|ref>` para casos especiales.

#### 4. Modos de review configurables
- **paranoid**: Cero concesiones. Debe cumplir Quality Gate completo y todas las reglas del reviewer
- **strict**: Seguridad + bugs + rendimiento + tests
- **standard (default)**: Prioriza lo critico/importante (seguridad, bugs de impacto, tests). Tolera sugerencias no bloqueantes de estilo/refactors menores
...
#### 6.1 Resiliencia y limites operativos
- Limite de iteraciones global (`max_iterations`)
- Timeout por iteracion (coder/reviewer/scan) para evitar bloqueos
- Budget maximo opcional por sesion (USD/tokens/tiempo)
- Regla fail-fast: abortar si el mismo issue bloqueante se repite N veces sin progreso
- Checkpoints de sesion para poder reanudar con `kj resume <session-id>`
...
| `kj resume <session-id>` | Reanuda una sesion fallida/interrumpida desde su ultimo checkpoint | si | si | si | si |
...
```bash
# Overrides de kj.config.yml via flags
...
kj run "tarea" --base-branch main     # Base explicita para calcular diff
...
- Prioridad de carga: variables de entorno (`KJ_SONAR_TOKEN`, `OPENAI_API_KEY`, etc.) > `kj.config.yml`.
- Si `kj init` guarda secretos en disco, el fichero debe crearse con permisos `0600`.
- Los logs y reportes deben enmascarar tokens (mostrar solo prefijo/sufijo).
- `kj doctor` valida presencia/autenticacion sin imprimir secretos completos.
...
# 2. Consulta Quality Gate (criterio de bloqueo real)
curl -s -u "$KJ_SONAR_TOKEN:" \
  "http://localhost:9000/api/qualitygates/project_status?projectKey=$PROJECT_KEY"

# 3. Si Quality Gate = ERROR/WARN (segun perfil) → coder corrige → re-scan
# 4. Consulta issues para detalle de remediacion
curl -s -u "$KJ_SONAR_TOKEN:" \
  "http://localhost:9000/api/issues/search?projectKeys=$PROJECT_KEY&statuses=OPEN"
# 5. Si Gate pasa → reviewer
```
...
sonarqube:
  ...
  token: null                          # Opcional; mejor via env KJ_SONAR_TOKEN
  quality_gate: true                   # Fallar si no pasa el Quality Gate
  enforcement_profile: pragmatic       # pragmatic (default) | paranoid
  gate_block_on:                       # En pragmatic: que condiciones bloquean
...
- `pragmatic` (default): bloquea por condiciones criticas/importantes; ignora ruido menor (p.ej. recomendaciones de estilo no bloqueantes).
- `paranoid`: exige Quality Gate completo sin excepciones.
...
### Contrato de salida del reviewer (JSON)

Para que el loop sea determinista, `kj` exige este schema minimo:
  ...
  "blocking_issues": [
    {
      ...
      "file": "src/auth.ts",
      "line": 88,
      "description": "Token JWT sin validacion de expiracion",
      "suggested_fix": "Validar exp y rechazar tokens vencidos"
    }
  ],
  "non_blocking_suggestions": [],
  "summary": "Faltan validaciones en auth",
  "confidence": 0.86
}
```

Reglas:
- `approved=true` solo si `blocking_issues` esta vacio.
- Si el reviewer no devuelve JSON valido, `kj` reintenta una vez con prompt de correccion de formato.
...
| Dato | Almacenamiento |
|------|---------------|
| Configuracion | `kj.config.yml` en el proyecto del usuario |
...
│   │   ├── resume.js          # kj resume - reanudar sesion desde checkpoint
...
| **CLIs de IA instalados segun comando** | - | Ver matriz abajo |
...
#### CLIs de IA (segun comando)

Matriz minima:
- `kj run`: 2 CLIs (coder + reviewer)
- `kj code`: 1 CLI (coder)
- `kj review`: 1 CLI (reviewer)
- `kj plan`: 1 CLI
- `kj scan`: 0 CLIs (solo SonarQube)
...
| **Gemini CLI** | `npm install -g @google/gemini-cli` o `npx @google/gemini-cli` | `gemini --version` | Cuenta Google (gratis/Pro/Ultra) |
...
$ kj doctor
  ...
  AI CLIs:
    ...
    ✗ Aider not found
    2/4 available (enough for `kj run`) ✓
...
kj run "Fix bug en auth" --base-branch develop    # Baseline explicito para diff
...
# Reanudar si la sesion se interrumpe
kj resume s_2026-02-13_001
...
# Review settings
review_mode: standard            # default: standard (critico/importante)
max_iterations: 5                # Maximo de ciclos coder/reviewer
review_rules: ./review-rules.md  # Reglas custom para el reviewer
base_branch: main                # Rama base para merge-base del diff
...
reviewer_options:
  ...
  require_schema: true             # Exigir contrato JSON minimo
  model: null
...
sonarqube:
  ...
  token: null                          # Preferible via env KJ_SONAR_TOKEN
  quality_gate: true                   # Fallar si no pasa el Quality Gate
  enforcement_profile: pragmatic       # pragmatic (default) | paranoid
  gate_block_on:                       # Criterios de bloqueo en modo pragmatic
    - new_reliability_rating=E
    - new_security_rating=E
    - new_maintainability_rating=E
    - new_coverage<80
...
# Session limits
session:
  max_iteration_minutes: 20      # Timeout por iteracion
  max_total_minutes: 120         # Timeout total de sesion
  max_budget_usd: null           # null = sin limite economico
  fail_fast_repeats: 2           # Abort si issue bloqueante se repite sin progreso
```
...
- [Gemini CLI - Install](https://geminicli.com/docs/get-started/installation/)
