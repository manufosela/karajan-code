...
deadlock with good intentions. Six of them, found by running the method against
its own author for a day: the release check refusing the deploy it demanded,
board-sync refusing the second half of a card it had already accepted splitting,
the supervisor reading its own development as tampering, and a broken kj denying
the edit that would repair it. Each one had the same workaround, an escape that
switches the whole gate off, which is how a protection becomes a habit nobody
trusts.

### Added

- **A release item can name what repairs it** (KJC-BUG-0204, #1805): `release_check.items` accepts `remedied_by`, and `kj release check --json --for-command "<cmd>"` answers for one specific command. A red item whose remedy IS that command stops deciding the exit code; every other red still does. The lifted item stays red in the report, because the fact has not changed, and the gate says which one it lifted instead of exempting it in silence. A package publication is never excused: `npm publish` and `gh release create` are irreversible, whatever an item declares.
- **A flag cannot switch the commit gate off** (KJC-TSK-0854, #1819): the cross-AI review, the policy engine and the privacy scan all run in the commit hook, so `git commit --no-verify` skipped every one of them and left the diff with no verdict bound to it, and nothing stopped a session from typing it. It is denied now, along with every abbreviation git would accept (`--no-ver`, `-n`, a short-flag cluster carrying it), and so is naming `core.hooksPath` through any of git's doors to that key (`-c`, `config`, `--config-env`, `GIT_CONFIG_KEY_n`, `GIT_CONFIG_PARAMETERS`), because moving it switches off every hook at once. Reading it is denied too: exempting reads let one be appended behind the change. `KJ_ALLOW_NO_VERIFY=1` is the human's way through, sealed like every other escape, and the `--no-verify` that `kj harden --commit` uses internally is untouched because it never goes through a tool call.
- **The panel tally counts both sides** (KJC-TSK-0873, #1818): the deviation alone could not say whether the panel is honoured almost always or almost never, and those are different situations. `kj code` now seals the compliance where it happens, and `kj report` prints the pair. A project with no panel prints nothing. On the way, `withBrainRecovery` started carrying the provider that ACTUALLY ran: after a fallback the caller believed the declared coder had written, which would have counted another agent's work as yours.
- **The host writing with another coder declared is recorded** (KJC-TSK-0868, #1816): announcing the panel at session start was not enough, because when the host writes the code anyway the only way to find out was by accident. The turn now ends with that fact in the decision log (`exempt`, chokepoint `panel`), naming the declared coder and the host that wrote instead, in the same hash-chained log the escapes use. Once per session, never blocking: the card asks to be told, not to be stopped. Nothing is recorded when the panel was honoured, when nobody declared a coder, or when the host cannot be identified.
- **`kj sentinel board-gate`** (KJC-BUG-0198, #1812): which pending board moves actually block, decided from verifiable facts rather than the agent's word. The guard asks, kj answers, so the rule travels with an npm install instead of waiting for the supervisor to be re-sealed.

### Changed

- **A card split across PRs stops blocking on its own first half** (KJC-BUG-0198, #1812): board-sync recorded one pending move per merged PR and refused every advance until that card reached a closing state, which assumes one PR per card. The project's own rule says the opposite: a card that does not fit in ~150 lines is split. A pending whose card has another OPEN pull request is now carried, said out loud, and left in place, so the Stop gate still refuses to end a turn with work nobody recorded. A branch named after the card is not enough, because the merged branch carries that name too. If `gh` cannot answer, everything blocks as before.
- **The supervisor's seal survives a reinstall** (KJC-BUG-0193, #1810): `kj harden` rewrote the installed guards unconditionally, and that is the command a session reaches for after editing the guard templates. So the act of unblocking replaced content a human had sealed with content nobody sealed, and destroyed the seal that made unblocking unnecessary. A guard holding exactly what the provenance in `HEAD` covers is now left alone, and harden names whose job it is to move it: `kj harden --commit`, which advances guards and seal together.
- **The publication guard recognises the real command** (KJC-BUG-0204, #1805): it matched the verb only when its words were adjacent, so `firebase --account a@b --project p deploy --only hosting:main`, which is what a deploy actually looks like, was never intercepted. The words must now appear in order, read by their basename, with separators splitting them: `(npm publish)`, `npm --registry https://r.example publish` and `/usr/bin/npm publish` are all publications, and none of them were before.

### Fixed

- **The release gate stops blocking the one action that would turn it green** (KJC-BUG-0204, #1805): the checklist demanded a landing deployed with the new version, the guard treated that deploy as a publication, and the only way through was `KJ_ALLOW_RELEASE=1`, which does not lift that one item: it switches the whole gate off, including the protection over the publish that comes thirty seconds later. The guide even documented that as the legitimate path.
- **Tampering and drift stop being the same thing** (KJC-BUG-0197, #1802, #1803): tamper detection compared the installed guards against the templates of the tree being edited, so developing the supervisor read as attacking it and left the session without tools until a human reinstalled. Two layers close it. The supervisor's seal now covers the supervisor's own guards, which it never did (it sealed `.karajan/hooks` only, and a deleted guard simply vanished from the seal instead of being recorded as deleted). And the check reads that provenance from git, never from the working tree, with each hash bound to its path: what a human sealed is drift and does not block, what nobody sealed is tampering and fails closed. No git, no commit or no seal forgives nothing.
- **The audit role goes through the brain like every other** (KJC-BUG-0194, #1801): it was the only role calling the model directly, so a quota wall or a retired model killed the whole audit instead of taking the next declared candidate. It now runs through the recovery wrapper with the role's fallback chain and a one-shot policy: a quota error takes the next candidate at once rather than sleeping inside a CLI command, and a failure that exhausts the chain says what was tried and in what order.
- **One LOC budget instead of three that disagreed** (KJC-BUG-0205, #1804, and KJC-BUG-0208, #1808): the rule that caps code growth was implemented three times and answered differently each time, so a documentation-only PR was warned at 234 lines locally and counted 104 in CI, while the workflow kj generates for other projects exempted every `.md`, including the files that instruct the agent. One module decides now: generated output never counts because nobody wrote it, human documentation never counts wherever the project keeps it, and AI-rule files always count because they enter the agent's context on every run. The policy invariant that fed a fourth number reads that same module, so the same diff no longer prints 174 and 202 two lines apart.
- **A kj that cannot start is not a policy deny** (KJC-BUG-0207, #1813): with kj linked to the tree, a transient syntax error in `src/` made the policy evaluator fail, the guard denied by default, and the session lost Edit, Write and Bash at once, which is exactly the set of tools needed to repair the file. The remedy it printed could not start either. When the evaluator fails because kj itself cannot load, the guard now allows exactly one thing, editing the file the diagnostic names, and keeps denying everything else, because opening the gate would be a bypass anyone could trigger by breaking a module on purpose.
- **A file git does not know yet is still new to the RAG** (KJC-BUG-0210, #1815): the rag-first gate called a file new only while it did not exist on disk, so the moment the session created it the next edit demanded an answer about a file the index cannot hold: `kj rag index --since` works off git diffs and the file is not committed, the sibling heuristic does not help in a new directory, and no query can return it. The only ways out were the escape or moving the file to please the gate, and both happened today. A file git does not know has not been indexed, and that is checkable: untracked now counts as new, which still requires the session to have consulted the RAG at all. Tracked files are unchanged, git is only asked on the path that was about to deny, and anything other than a clear answer keeps blocking.
- **A TypeScript project keeps its language rules** (KJC-BUG-0206, #1809): the per-language guidelines had an entry for JavaScript and the stack detector reports `typescript` for a repo with a tsconfig, so `kj harden` wrote the language-agnostic core alone and ERASED six lines of rules the project already had. It hit every TypeScript project, which is the most common case after JavaScript. TypeScript now resolves to the JavaScript block, and removing rules is said out loud: a block poorer than the one installed makes harden warn instead of reporting "updated" as if it were an improvement.
...
- **The sealed supervisor no longer blocks its own release** (KJC-BUG-0178, #1770): with the seal merged, cutting this very version went red — `[defaults.supervisor.write]` on the hook and on its own provenance. `kj review` and `kj policy check` already lift what the sealed provenance backs (ADR 0009); the release check called the policy engine directly and never did, so the first version after a human `kj harden --commit` was denied by its own seal. Nothing is relaxed: only a file whose bytes match both the sealed sha and the canonical render is lifted, and the provenance itself only when every entry verified.
...
- **The installers end with ONE instruction: `kj go`** (MGL-B, KJC-TSK-0809): the one-gesture installer already existed and already met the epic's bar — `curl -fsSL karajancode.com/install.sh | sh` (and `install.ps1` on Windows) with checksum-verified Node auto-provisioning and a standalone SEA route. What expelled a muggle was the EPILOGUE: "run kj doctor, then kj install-tools" is jargon. Every finish line of both installers now says `Listo. Ahora escribe:  kj go` — with the technical commands demoted to an "avanzado" note for those who want them.

- **The board speaks plain language when `kj go` opens it** (MGL-C, KJC-TSK-0810): the muggle's window renders in maggle mode — kanban columns **Por hacer / En marcha / Hecho**, header **Tu trabajo**, counts in **tareas**, the nav folded to **Tablero | Proyectos | Más** (with the way back to expert mode), the ⚡ launcher leading with **«Pedir trabajo nuevo»** (one plain textarea, a clear confirm of what will happen, the technical launcher one click behind «Más opciones…»), the run-log panel presented as **Actividad**, and errors that never show a stacktrace alone: plain headline, an actionable next step, the raw message collapsed as technical detail. The jargon survives as tooltip — plain first, technical second. The mode arrives via `/?maggle=1`, persists in `localStorage`, and a dev opening `localhost:4000` directly sees no change. `boardUrl(port, path)` is the new primitive the `open` action honors.

- **ADR 0008 (PROPOSED — the user decides): the single window via embedded pty** (MGL-D spike, KJC-TSK-0811): three approaches evaluated for embedding the agent conversation inside the board. The proposal: embed the REAL interactive terminal (node-pty + xterm.js, WebSocket bound to 127.0.0.1 with a session token) because the process stays Claude Code interactive and the Sentinel's synchronous hooks stay guaranteed; an own chat over an agent SDK is discarded while the Sentinel depends on the interactive host's hooks; a desktop wrapper remains a possible future shell over the winner. Nothing is implemented until the ADR is accepted.
...
- **The Steward's execution modes — the project's user chooses** (STW-E, KJC-TSK-0793, epic KJC-PCS-0081, closing the epic): decay happens precisely on the days nobody looks, so the watcher cannot live only inside a session. On-demand (`kj steward sweep`) and ON-RESUME by default with nothing to install — `kj resume` sweeps only when the project ADOPTED the Steward and only if the report went stale (`--if-stale <days>`: resuming with a fresh report does not re-sweep, and a failed sweep never stops a resume). The scheduled GitHub Action is OPT-IN (`steward.action: true`): `kj harden` generates `kj-steward.yml` — weekly cron plus `workflow_dispatch`, actions pinned to SHAs like every generated workflow — which runs the sweep where the repo lives, publishes the SAME report file every other mode writes (one source of truth, never two competing) and commits the proposed cards; a broken invariant never stops the report from publishing. A local daemon was rejected by the card itself: it dies with the machine and covers nobody else.
- **Phantom coverage is TWO detectors** (KJC-TSK-0800 part 1, epic KJC-PCS-0081): a phantom test is worse than no test — it adds to the count, feels like a net, and covers nothing. The CALL-GRAPH detector catches a unit test whose member calls all land on unreachable members (one call to a live member absolves: that test covers something), and the LITERAL-CROSSING detector catches the case the graph cannot see — GREBLA's `hierarchy.spec` called no dead method, it looked for an aria-label that only existed inside an unreachable one: literals the test looks for that ONLY live inside unreachable spans are phantoms, named with the literal and where it lives; a literal also present in reachable code is never reported. Both inherit the reachability perimeter: a not-observable analysis makes the detector not observable — no test is accused on an unreliable basis. And part 2 closes the card: the TEMPORAL signature — zero static analysis, and it would have been enough for GREBLA: a test failing the SAME way for longer than the threshold is suspected phantom, not bug (a bug gets fixed or reverted; a phantom fails identically forever); kj holds no per-test history, so the source is injected and said. The `phantom-coverage` invariant now consumes the detectors' output — phantoms break, an empty run is ok, and no output stays not-observable: never ok by absence of the detector's run.
- **Karajan Console: `/api/audit` waits for the sealing queue to drain** (KJC-TSK-0799, from tribbu-atlas' field report): reading the audit right after a burst returned fewer entries while the queue drained — nothing lost, but an audit that LOOKS incomplete is worse than a slow one. The endpoint now waits for every in-flight seal before answering (`audit.drained()`, re-checking for entries that queued behind the one it waited for); a failed upload never hangs the query and its entry is never counted as sealed. The README explains the one-at-a-time sealing and the drain.
- **The Sonar scanner respects the repo's own `sonar-project.properties`** (KJC-BUG-0156, issue #1543, field report): in a monorepo with the canonical file and no root `src/`, kj's own `-Dsonar.sources` (default `src,public,lib`) overrode it on every scan — writing the file was useless, the scan failed, and the pre-gate skipped with a warning that read like config noise while eight straight PRs said APPROVED over a dead quality gate. Now, when the repo brings the file, kj's layout opts stand down (only its own ignore rules travel; the precedence is said out loud, never silent), and a pre-gate that could NOT run is typographically distinct from a chosen skip: `✗ sonar pre-gate UNAVAILABLE — the quality gate did NOT run on this diff` — still fail-open, but impossible to read as one more warning.
