...
const PLAYBOOK_BODY = `# Karajan method (v4)
...
export function renderPlaybook(_options = {}) {
 ...
 */
export async function installPlaybook({ projectDir, target = "both", version = "1" }) {
  ...
  for (const file of TARGET_FILES[target]) {
    ...
    const { content, action } = upsertManagedBlock({
      source, blockId: "playbook", version, body: renderPlaybook({ target }), style: "html",
