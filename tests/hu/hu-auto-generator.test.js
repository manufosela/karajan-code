...
describe("generateHuBatch", () => {
  ...
  // Resilience audit follow-up — sesgo-de-stack PR 2/3.
  // Until now every autogenerated HU (Python, Go, Rust, anything)
  // received hardcoded `npm install` + `npx vitest run` acceptance
  // tests, so the coder ended up installing vitest in a pure-Python
  // repo to satisfy the contract. The HU template is now language-aware.
  describe("language-aware HU templates", () => {
    it("python stack → pytest acceptance tests, not vitest", () => {
      const batch = generateHuBatch({
        originalTask: "Build a Python service",
        subtasks: ["Implement /health endpoint"],
        stackHints: ["fastapi", "pytest"],
        isNewProject: true,
      });
      const setup = batch.stories[0];
      expect(setup.title).toMatch(/python/i);
      expect(setup.acceptance_tests.join("\n")).toMatch(/python -m pytest/);
      expect(setup.acceptance_tests.join("\n")).not.toMatch(/vitest|npm install/);
      const task = batch.stories[1];
      expect(task.acceptance_tests.join("\n")).toMatch(/python -m pytest/);
      expect(task.acceptance_tests.join("\n")).not.toMatch(/vitest/);
    });

    it("go stack → go test, not vitest", () => {
      const batch = generateHuBatch({
        originalTask: "Build a Go service",
        subtasks: ["Implement /ping"],
        stackHints: ["gin"],
        isNewProject: true,
      });
      const setup = batch.stories[0];
    ...
    it("rust stack → cargo test, not vitest", () => {
      const batch = generateHuBatch({
        originalTask: "Build a Rust CLI",
        subtasks: ["Parse args"],
        stackHints: ["cargo"],
        isNewProject: true,
      });
