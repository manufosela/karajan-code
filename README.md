...
> **v2.7.1 released** — Patch that restores SEA binary publishing (broken since v2.4.1). GitHub Releases now ship `linux-x64`, `darwin-arm64` and `win-x64` binaries plus SHA256 checksums again. Carries the full v2.7.0 feature set: [`addyosmani/agent-skills`](https://github.com/addyosmani/agent-skills) as Karajan's first-source process catalog (20+ lifecycle workflows — TDD, code-review-and-quality, security-and-hardening, performance-optimization, git-workflow-and-versioning, CI/CD, debugging-and-error-recovery, spec-driven-development, planning-and-task-breakdown…) injected per role **before** OpenSkills stack skills, shallow-cloned into `~/.karajan/agent-skills/` and refreshed weekly via `git pull`. Role → slug map: `tester → test-driven-development`, `reviewer → code-review-and-quality`, `security → security-and-hardening`, `architect → spec-driven-development + api-and-interface-design`, `coder → incremental-implementation + debugging-and-error-recovery`... New CLI: `kj skills sync-addyosmani`, `kj skills list-addyosmani`. New config: `skills.sources`, `skills.addyosmani.{enabled,refreshDays,repoUrl}`. See [CHANGELOG.md](./CHANGELOG.md) for full history and [MIGRATION-v2.md](./MIGRATION-v2.md) for v1→v2 breaking changes.
