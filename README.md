...
> **v2.7.0 released** — [`addyosmani/agent-skills`](https://github.com/addyosmani/agent-skills) is now Karajan's first-source process catalog. 20+ lifecycle workflows (TDD, code-review-and-quality, security-and-hardening, performance-optimization, git-workflow-and-versioning, CI/CD, debugging-and-error-recovery, spec-driven-development, planning-and-task-breakdown...) are injected per role **before** OpenSkills stack skills. Shallow-cloned into `~/.karajan/agent-skills/` on first use, refreshed weekly via `git pull`, degrades silently if git is missing. Role → slug map: `tester → test-driven-development`, `reviewer → code-review-and-quality`, `security → security-and-hardening`, `architect → spec-driven-development + api-and-interface-design`, `coder → incremental-implementation + debugging-and-error-recovery`... Task-text triggers add slugs on top (e.g. "performance" → `performance-optimization`). New CLI: `kj skills sync-addyosmani`, `kj skills list-addyosmani`. New config: `skills.sources`, `skills.addyosmani.{enabled,refreshDays,repoUrl}`. See [CHANGELOG.md](./CHANGELOG.md) for full history and [MIGRATION-v2.md](./MIGRATION-v2.md) for v1→v2 breaking changes.
