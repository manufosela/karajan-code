...
> **v2.9.0 released** — Audit overhaul release. `kj audit` becomes a stack-aware, two-phase analysis tool. Highlights: (1) **Two-phase mode**: deterministic findings (basalCost + Sonar + OSV + Semgrep + WebPerf) print first; you decide whether to spend tokens on the LLM analysis with `Continue with LLM analysis? [y/N]`. New `--deterministic-only` flag for zero-token runs. (2) **Three new deterministic security collectors**: SonarQube findings as ground truth in the prompt, OSV-Scanner for dependency CVEs (broader DB than `npm audit`), Semgrep for SAST (XSS, SQLi, taint flow, hardcoded secrets — equivalent to `snyk code` but free and local). (3) **Stack-aware prompt**: detects frontend/backend/fullstack and filters dimension heuristics — no more N+1 query nags on Astro projects, no more bundle-size nags on Express APIs. (4) **Accessibility dimension** auto-activates for frontend projects (WCAG 2.x: alt text, labels, ARIA, focus management, contrast hints). (5) **WebPerf section** with frontend perf hints (lazy loading, image format, render-blocking, font-display, critical CSS) plus optional Core Web Vitals verdict integration. (6) **`--report-file` flag** persists audits to `.md` or `.json` with reproducibility header (timestamp, branch, commit, flags). `$KJ_AUDIT_REPORT_DIR` env var for CI. (7) **Token/cost transparency**: every audit ends with provider + model + tokens + estimated USD cost. (8) **CLI/MCP parity bug fixed** — CLI now drives `AuditRole` like MCP does; both paths produce identical prompt content. Plus repo health: 228 dead exports → 3 in 5 atomic PRs, and the `kj audit` detector itself now matches knip ground truth (166 → 4 false positives, 99.7% reduction). 13 audit PRs + 5 dead-export PRs + 106 new tests; full suite 4305/4305 passing. See [CHANGELOG.md](./CHANGELOG.md) for the full punch list.
